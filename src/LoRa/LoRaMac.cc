...
void LoRaMac::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        radio->setRadioMode(IRadio::RADIO_MODE_RECEIVER); // HEPSI ACIK BASLASIN
}
...
void LoRaMac::handleSelfMessage(cMessage *msg)
{
    if(msg == endTransmission){
        EV << "endTransmission Signal has come.\n";
        popTransmissionQueue();
        //turnOnReceiver();
    }
    else if(msg == droppedPacket){
        EV << "PACKET DROPPED\n";
    }
    else{
...
void LoRaMac::handleUpperPacket(cPacket *msg)
{
    ...
    LoRaAppPacket* deneme = check_and_cast<LoRaAppPacket *> (msg);
    if(deneme->getMsgType() == JOIN_REQUEST){
        EV << "SENKRONIZE\n";
    }
    lastSendedMessage = deneme;
    ...
    if(deneme->getMsgType() == ACKNOWLEDGE){
        frame->setReceiverAddress(cInfo->getDest());
    }
    else{
        frame->setReceiverAddress(parentAddr);
    }
    ++sequenceNumber;
...
void LoRaMac::handleLowerPacket(cPacket *msg)
{
   /* if( (fsm.getState() == RECEIVING_1) || (fsm.getState() == RECEIVING_2)) {
    ...
    if(isSynchMessage(msg) && droppedPacket != msg){
        EV << "RECEIVED SENDING UP";
        sendUp(decapsulate(check_and_cast<LoRaMacFrame *>(msg)));
        //sendUp(msg);
    }
    else if(isDataMessage(msg) && droppedPacket != msg && isForUs(frame)){
        EV << "GETTING DATA"; //recv timeına bakarak kontrol et gectiyse dropla
        sendUp(decapsulate(check_and_cast<LoRaMacFrame *>(msg)));
    }
    else if(isAcknowledgeMessage(msg) && droppedPacket != msg && isForUs(frame)){
        EV << "RECEIVING ACKNOWLEDGE";
        sendUp(decapsulate(check_and_cast<LoRaMacFrame *>(msg)));
    }
...
void LoRaMac::handleWithFsm(cMessage *msg)
{
    ...
    FSMA_Switch(fsm)
    {
        FSMA_State(IDLE) //Point 2
        {
            ...
            /*FSMA_Event_Transition(Idle-Receiving_SYNC,
                    isLowerMessage(msg)&&isSynchMessage(msg),
                    RECEIVING_SYNC,
            ); */
        }
        FSMA_State(TRANSMIT) //Point 3
        {
        ...
        FSMA_State(RECEIVING_1) // Point 1
        {
...
void LoRaMac::receiveSignal(cComponent *source, simsignal_t signalID, long value, cObject *details) //Nasıl tetikleniyor acaba
{
    Enter_Method_Silent();
    if (signalID == IRadio::receptionStateChangedSignal) { //Buda reception için
        IRadio::ReceptionState newRadioReceptionState = (IRadio::ReceptionState)value;
        if (receptionState == IRadio::RECEPTION_STATE_RECEIVING) { //ALINIYOR DEMEKMIS
        ...
        handleSelfMessage(droppedPacket);
    }
    else if (signalID == IRadio::transmissionStateChangedSignal) {
        IRadio::TransmissionState newRadioTransmissionState = (IRadio::TransmissionState)value;
        if (transmissionState == IRadio::TRANSMISSION_STATE_TRANSMITTING && newRadioTransmissionState == IRadio::TRANSMISSION_STATE_IDLE ) {
            //handleWithFsm(endTransmission); //Burayi hallet
            scheduleAt(simTime(),endTransmission);
            if(lastSendedMessage->getMsgType() == ACKNOWLEDGE){
                numberOfLeftDataMessages--;
                if(!numberOfLeftDataMessages){
                    numberOfLeftDataMessages = -1;
                    radio->setRadioMode(IRadio::RADIO_MODE_SLEEP);
                }
                else{
            ...
            else if(lastSendedMessage->getMsgType() != JOIN_REQUEST){
                radio->setRadioMode(IRadio::RADIO_MODE_RECEIVER);
            }
             //BURA DEĞİŞECEK
            //Tam buranın senkronizasyondan sonra kapatılmasını handle etmek gerek.
            //Senkronizasyona özel burası göndermemeli
        }
...
LoRaMacFrame *LoRaMac::encapsulate(cPacket *msg)
{
    ...
    frame->setByteLength(headerLength); // Convenience için buraya dikkat
    frame->setArrival(msg->getArrivalModuleId(), msg->getArrivalGateId()); //Bu nedir acaba yya
...
bool LoRaMac::isSynchMessage(cMessage *msg){
    //TODO
    cMessage *duplicate = msg->dup();
    LoRaAppPacket *packet = check_and_cast<LoRaAppPacket *> (decapsulate(check_and_cast<LoRaMacFrame *>(duplicate)));
    bool returningValue = packet->getMsgType() == JOIN_REQUEST;
   // delete duplicate;
    return returningValue;
}

bool LoRaMac::isDataMessage(cMessage *msg){
    cMessage *duplicate = msg->dup();
    LoRaAppPacket *packet = check_and_cast<LoRaAppPacket *> (decapsulate(check_and_cast<LoRaMacFrame *>(duplicate)));
    bool returningValue = packet->getMsgType() == DATA;
    // delete duplicate;
    return returningValue;
}

bool LoRaMac::isAcknowledgeMessage(cMessage *msg){
    cMessage *duplicate = msg->dup();
    LoRaAppPacket *packet = check_and_cast<LoRaAppPacket *> (decapsulate(check_and_cast<LoRaMacFrame *>(duplicate)));
    bool returningValue = packet->getMsgType() == ACKNOWLEDGE;
    //delete duplicate;
    return returningValue;
}
...
void LoRaMac::setParentAddress(DevAddr address2){
    this->parentAddr = address2;
}
